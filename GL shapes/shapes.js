...
function createPacman(gl, segments) {
    ...
    var pi = Math.PI;
    let beta = (2 * pi) / segments

    phase = theta / segments

    for (i = 0; i < segments + 2; i++) {

        if (i > phase && i < (segments - phase)) {
            console.log("entry through i:" + i)
            let x = radius * Math.cos(beta * i)
            let y = radius * Math.sin(beta * i)
            let z = 0;
